...
export async function exportCustomerViewPDF(
  ...
  doc.setFont(undefined, 'normal')
  doc.setFontSize(7.5)
  const summary = `Explore potential for Scope 3 emissions reduction. Illustrative financial exposure based on entered parameters. Technical qualification and supply arrangements require confirmation.`
  ...
  doc.setTextColor(BRAND_GREEN)
  doc.setFont(undefined, 'bold')
  doc.text('ESTIMATED IMPACTS', margin, yPos)
  yPos += 4
  doc.setTextColor(BRAND_GRAY)
  doc.setFont(undefined, 'normal')
  doc.setFontSize(7.5)
  const benefits = [
    `• Scope 3: ~${(output.co2Saved || 0).toLocaleString()} t CO₂ reduction annually`,
    `• Carbon Value: ~€${(output.indicativeCarbonValue || 0).toLocaleString()} illustrative annual exposure`,
    `• Supply: ${deal.supplyReliability || '—'} reliability rating (pending confirmation)`,
    `• Technical: Compatibility requires validation before implementation`,
  ]
  ...
  doc.setTextColor(BRAND_GREEN)
  doc.setFont(undefined, 'bold')
  doc.text('STATUS & REQUIREMENTS', margin, yPos)
  yPos += 4
  doc.setTextColor(BRAND_GRAY)
  doc.setFont(undefined, 'normal')
  doc.setFontSize(7.5)
  const certs = [
    `• Certification: ${deal.certificationStatus || '—'} (pending confirmation)`,
    `• Proof Items: ${deal.proofItemsAvailable || 0} of ${deal.proofItemsRequired || 0} available`,
    `• Technical Qualification: ${deal.technicalQualificationStatus || '—'} (validation required)`,
  ]
  certs.forEach((cert) => {
